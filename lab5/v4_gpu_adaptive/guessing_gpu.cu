// ============================================================
// v4_gpu_adaptive/guessing_gpu.cu — 进阶3: 按总value量触发批次
// 基于 v3 pipeline, 仅改动批量触发条件:
//   v3: if (candidates.size() >= 16) → launch
//   v4: if (total_vals >= 200000)    → launch
// ============================================================
...
__global__ void gen_kernel_batch(
    ...
    int pt_id = blockIdx.x, tid = threadIdx.x;
...
// ============================================================
struct AsyncTask {
    bool in_flight = false; GpuBuf* buf = nullptr;
    int total_vals=0, slot_len=0;
    vector<char> h_out; vector<int> h_lens;
};
...
static void drain_async(PriorityQueue& q) {
    ...
    size_t ob = (size_t)g_task.total_vals * g_task.slot_len;
    g_task.h_out.resize(ob); g_task.h_lens.resize(g_task.total_vals);
    CUDA_CHK(cudaMemcpy(g_task.h_out.data(), g_task.buf->d_output, ob, cudaMemcpyDeviceToHost));
    ...
    for (int i=0;i<g_task.total_vals;i++) {
        int len = g_task.h_lens[i]>0 ? g_task.h_lens[i] : (int)strlen(&g_task.h_out[i*g_task.slot_len]);
...
// ============================================================
// CalProb / init / NewPTs / build_prefix / Generate (同 v1-v3)
// ============================================================
...
void PriorityQueue::Generate(PT pt) {
    CalProb(pt); string prefix; segment* ls; int n;
...
// ============================================================
// ★ v4 核心: PopNextAdaptive
//   基于 v3 异步流水线, 仅改动批量触发条件:
//   v3: if (candidates.size() >= 16) → launch
//   v4: if (total_vals >= 200000)    → launch
// ============================================================
void PriorityQueue::PopNextAdaptive() {
    ...
    // ── Step 2: 收集 PT, 阈值过滤 ──
    int n = min(64, (int)priority.size());
    ...
    // ── Step 3: ★ v4 关键改动 — 按总 value 数触发, 而非固定 PT 个数 ──
    bool should_launch = (total_vals >= GPU_VALUE_TRIGGER);

    if (should_launch && !batch_pts.empty()) {
        // 扁平化
...
void PriorityQueue::PopNext() { PopNextAdaptive(); }

void gpu_cleanup() {
    drain_async(*((PriorityQueue*)nullptr));
    g_a.free_all(); g_b.free_all();
}
