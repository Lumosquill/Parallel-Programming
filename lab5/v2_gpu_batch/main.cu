// ============================================================
// v2_gpu_batch/main.cu — 进阶1: 多PT批处理
// 用法: 可改 batch_size 参数测试不同批次大小
// ============================================================
...
int main()
{
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (ss.str() != test_hashes[i]) {
            cout << "MD5Hash test failed!" << endl; return 1;
        }
    }
    ...
    cout << "Starting with batch_size=" << BATCH_SIZE << "..." << endl;

    int curr_num = 0, history = 0;
    auto t_start = system_clock::now();

    while (!q.priority.empty()) {
        q.PopNextBatch(BATCH_SIZE);      // ★ 多PT批处理
