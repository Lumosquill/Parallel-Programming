// ============================================================
// v2_gpu_batch/PCFG.h — 进阶1: 多PT批处理
// 在 v1 基础上增加 PopNextBatch()
// ============================================================
#pragma once
...
public:
    ...
    void train(string train_path); void store(string store_path); void load(string load_path);
...
#define GPU_BATCH_MAX   64       // ★ v2新增: 每批最多PT数
...
public:
    ...
    void Generate(PT pt);          // CPU版
    void GenerateGPU(PT pt);       // GPU单PT版 (保留)

    void PopNextBatch(int bsize);  // ★ v2核心: 多PT批处理
    void PopNext();                // 默认batch=1
