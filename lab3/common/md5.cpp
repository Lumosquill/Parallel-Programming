...
/**
 * StringProcess:预处理阶段 将单个输入字符串转换成 MD5计算所需的消息数组（MD5算法能处理的格式）
 * @param input 输入
...
/**
 * MD5Hash: 核心计算 用预处理后的单个输入字符串数据算出最终 MD5值 
 * @param input 输入
...
void MD5Hash(string input, bit32 *state)
{
	...
	int n_blocks = messageLength[0] / 64; // 分块处理,每个block为512bit，即64Byte

	// bit32* state= new bit32[4]; 这四个初始值是MD5算法的固定初始值，分别对应A、B、C、D四个缓冲区
	state[0] = 0x67452301;
	state[1] = 0xefcdab89;
	state[2] = 0x98badcfe;
	state[3] = 0x10325476;
	...
	for (int i = 0; i < n_blocks; i += 1)
	{
		...
		for (int i1 = 0; i1 < 16; ++i1)
		{
			x[i1] = (paddedMessage[4 * i1 + i * 64]) |
					...
					(paddedMessage[4 * i1 + 3 + i * 64] << 24);// 将每4个byte合并成一个32bit的word，注意是小端格式
		}
		...
		// 下面的FF、GG、HH、II函数分别对应MD5算法中的四轮操作，每轮操作中对a、b、c、d进行更新
		
		/* Round 1 */
		FF(a, b, c, d, x[0], s11, 0xd76aa478);
		// 这里的s11等是MD5算法中定义的常量，分别对应每轮操作中的左移位数;
		// 0xd76aa478等是MD5算法中定义的常量，分别对应每轮操作中的加数
